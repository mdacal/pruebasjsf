...
public class PersonaDao implements Dao<Persona> {
	...
	public Optional<Persona> get(long id) {
		...
		try {
			...
			if (rs.next()) {
				d = Optional.of(new Persona(rs.getInt("id"), rs.getString("nombre"), rs.getString("apellidos"),
						rs.getDate("f_nacimiento"), rs.getString("observaciones")));
			}
			sentencia.close();
	...
	public List<Persona> getAll() {
		...
		try {
			...
			ResultSet rs = sentencia.executeQuery();
			while (rs.next()) {
				listaPersonas.add(new Persona(rs.getInt("id"), rs.getString("nombre"), rs.getString("apellidos"),
						rs.getDate("f_nacimiento"), rs.getString("observaciones")));
			}
			sentencia.close();
		} catch (SQLException e) {
	...
	public void update(Persona persona) {
		String sql = "UPDATE " + NOM_TABLA
				+ " SET nombre= ?, apellidos = ?,f_nacimiento=?, observaciones=?  WHERE id = ? ";
		PreparedStatement sentencia;
		try {
			sentencia = conexion.prepareStatement(sql);
